...
/* ══════════════════════════════════════
   1. TRADER DNA PROFILING SYSTEM/* ═══════════════════ v5 UPGRADE ═══════════════════ */
/* ═══════════════════════════════════════════════════════════════
   TRADEBABY PRO v5 — MASSIVE UPGRADE
   All new systems — fully offline
   ═══════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════
   TTS ENGINE — Web Speech API (no API key, works on all browsers)
   Functions used by: float chatbot, mentor screen, lesson screen
   ═══════════════════════════════════════════════════════════════ */
let _ttsEnabled  = false;
let _ttsVoice    = null;
let _ttsSpeakBtn = null;
...
function ttsSpeak(html, btn) {
  ...
  if (!text) return;
  if (btn) { btn.innerHTML = _ttsIcon(true); btn.title = 'Stop'; _ttsSpeakBtn = btn; }
  const utt = new SpeechSynthesisUtterance(text);
  if (_ttsVoice) utt.voice = _ttsVoice;
  utt.rate = 1.05; utt.pitch = 1.0; utt.volume = 1.0;
  utt.onend = utt.onerror = function() { _ttsResetBtns(); };
  speechSynthesis.speak(utt);
}

function ttsSpeakMsg(btn, html) {
  if (_ttsSpeakBtn === btn && window.speechSynthesis && speechSynthesis.speaking) {
    speechSynthesis.cancel(); _ttsResetBtns(); return;
  }
  ttsSpeak(html, btn);
}
...
function _ttsMakeBtn(html, extraStyle) {
  ...
  btn.addEventListener('mouseleave', function() { if (_ttsSpeakBtn !== this) this.style.opacity = '0.5'; });
  btn.addEventListener('touchstart', function() { this.style.opacity = '1'; }, { passive: true });
  btn.onclick = function(e) { e.stopPropagation(); ttsSpeakMsg(this, html); };
  return btn;
}
...
function toggleFloatTTS() {
  ...
  if (_ttsEnabled) {
    ...
    btn.title = 'Auto-read ON — click to turn off';
    btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>';
  } else {
    btn.style.background = 'rgba(255,255,255,0.2)';
    btn.title = 'Auto-read messages aloud';
    btn.innerHTML = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#fff" stroke-width="2" stroke-linecap="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>';
...
function addFloatMsg(role, text) {
  ...
  wrap.style.cssText = 'display:flex;flex-direction:column;gap:2px;align-items:' + (role === 'bot' ? 'flex-start' : 'flex-end');
  ...
  if (role === 'bot') {
    wrap.appendChild(_ttsMakeBtn(text, 'margin-left:4px;'));
  }
  ...
  if (role === 'bot' && _ttsEnabled) {
    const speakBtn = wrap.querySelector('.tts-speak-btn');
    ttsSpeak(text, speakBtn || null);
  }
}
